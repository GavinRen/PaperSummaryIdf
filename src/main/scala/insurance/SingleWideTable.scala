...
object SingleWideTable {
  ...
  def RawPremium(df: DataFrame, Validdate: String, UnderWriteFlag: String, chargeFlag: String, cancelFlag: String, CoinsFlag: String,endorseTimes:String, CoinsRate: String, sumPremium: String, rawPremium: String): DataFrame = {
    def getRawPremium(row: Row): Double = {
      if (row.getString(0)=="1" )
    }
    val getRawPremiumFunction = udf(getRawPremium _)
    df.withColumn(rawPremium, getRawPremiumFunction(struct(UnderWriteFlag, chargeFlag, cancelFlag, CoinsFlag, CoinsRate, sumPremium)))
  }
